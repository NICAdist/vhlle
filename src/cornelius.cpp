...
double *GeneralElement::get_centroid() {
 if (!centroid_calculated) calculate_centroid();
 return centroid;
}
...
double *GeneralElement::get_normal() {
 if (!normal_calculated) calculate_normal();
 return normal;
}
 ...
 *
 */
double *Line::get_start() { return corners[start_point]; }
 ...
 *
 */
double *Line::get_end() { return corners[end_point]; }
 ...
 *
 */
double *Line::get_out() { return out; }
...
void Polygon::calculate_centroid() {
 ...
 for (int i = 0; i < Nlines; i++) {
  ...
  double A_i = 0.5 * sqrt(pow(a[x2] * b[x3] - a[x3] * b[x2], 2.0) +
                          pow(a[x1] * b[x3] - a[x3] * b[x1], 2.0) +
                          pow(a[x2] * b[x1] - a[x1] * b[x2], 2.0));
 ...
 *
 */
int Polygon::get_Nlines() { return Nlines; }
 ...
 *
 */
Line **Polygon::get_lines() { return lines; }
...
void Polygon::print(ofstream &file, double *pos) {
 for (int i = 0; i < Nlines; i++) {
  double *p1 = lines[i]->get_start();
  double *p2 = lines[i]->get_end();
  file << pos[x1] + p1[x1] << " " << pos[x2] + p1[x2] << " "
       << pos[x3] + p1[x3];
  file << " " << pos[x1] + p2[x1] << " " << pos[x2] + p2[x2] << " "
       << pos[x3] + p2[x3];
  file << " " << pos[x1] + centroid[x1] << " " << pos[x2] + centroid[x2] << " "
 ...
 *
 */
Polyhedron::~Polyhedron() { delete[] polygons; }
...
bool Polyhedron::lines_equal(Line *l1, Line *l2) {
 ...
 for (int i = 0; i < DIM; i++) {
  ...
  if (diff1 > eps && diff2 > eps) return false;
 }
...
void Square::construct_lines(double E0) {
 ...
 // surface. Also find_outside() arranges cuts so that first two cuts form a
 // line
 // as defined in the algorithm (in case there are 4 cuts)
 ends_of_edge(E0);
 if (Ncuts > 0) find_outside(E0);
 ...
 *
 */
int Square::is_ambiguous() { return ambiguous; }
 ...
 *
 */
int Square::get_Nlines() { return Nlines; }
 ...
 *
 */
Line *Square::get_lines() { return lines; }
...
void Cube::construct_polygons(double value0) {
 ...
 if (ambiguous > 0) {
  ...
  do {
   if (Nlines - used < 3) {
    cout << "Error: cannot construct polygon from " << Nlines - used << " lines"
         << endl;
    exit(1);
   }
 ...
 *
 */
int Cube::is_ambiguous() { return ambiguous; }
 ...
 *
 */
int Cube::get_Nlines() { return Nlines; }
 ...
 *
 */
int Cube::get_Npolygons() { return Npolygons; }
 ...
 *
 */
Polygon *Cube::get_polygons() { return polygons; }
...
/**
 *
 * Here we construct polyhedrons from the polygons. First we check if the
 * surface
 * is ambiguous and if it is not amibiguous we add all polygons to a single
 * polyhedron. If surface is ambiguous we need to connect the polygons one by
 * one
 ...
 *
 */
int Hypercube::get_Npolyhedrons() { return Npolyhedrons; }
 ...
 *
 */
Polyhedron *Hypercube::get_polyhedrons() { return polyhedrons; }
 ...
 * @param [in] dex Length of the sides of the cube. Must contains as many
 * elements as
...
/**
 *
 * This initializes the printing of the surface elements into file. Final
 * elements
 ...
 *
 * @param [in] cube Values at the corners of the cube as a 2d table so that
 * value
 ...
 *
 * @param [in] cube Values at the corners of the cube as a 3d table so that
 * value
...
/**
 *
 * Finds the surface elements in 3-dimensional case and prints the actual
 * triangles
 ...
 * value
 *                  [0][0][0] is at (0,0,0) and [1][1][1] is at (dx1,dx2,dx3).
 * @param [in] pos  Absolute position at the point [0][0][0] in form
 * (0,x1,x2,x3).
 *
 */
...
void Cornelius::surface_3d(double ***cube, double *pos, int do_print) {
 ...
 for (int i = 0; i < Nelements; i++) {
  ...
  // If we want to print the actual triangles, which are found, we must do it
  // here
 ...
 *
 * @param [in] cube Values at the corners of the cube as a 4d table so that
 * value
...
/**
 *
 * Returns the centroid vectors as a 2d table with the following number of
 * indices
 * [number of elements][4]. If the dimension of the problem is smaller than
 * four,
 * first (4-dimension) elements are zero. Note that this function allocates
 * memory and
 ...
 * factors(sqrt(-g)) from the metric. Note that this function allocates memory
 * and
 * the user must free it!
 *
 * @return Table with dimensions [number of elements][4] containing the normal
 ...
 * Returns the centroid vectors as a 2d table with the following number of
 * indices
 * [number of elements][dimension of the problem]. Note that this function
 * allocates
 * memory and the user must free it!
 *
 * @return Table with dimensions [number of elements][dimensions] containing the
 * centroid
 *         vectors of the surface elements.
 *
 */
 ...
 *
 * @param [in] i Number of surface element whose centroid one wants to get.
 * Valid
...
double Cornelius::get_centroid_elem(int i, int j) {
 if (i >= Nelements || j >= cube_dim) {
  cout << "Cornelius error: asking for an element which does not exist."
       << endl;
  exit(1);
 }
...
/**
 *
 * Returns an element of the normal vector. This gives \sigma_\mu without
 * factors
